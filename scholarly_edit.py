...
# -*- coding: utf-8 -*-
"""
Created on Tue May 24 10:47:16 2016
...
_PUBLICATIONSEARCH = '/citations?view_op=top_venues&hl=en&vq={0}'
_JOURNALSEARCH = 'citations?hl=en&view_op=search_venues&vq={0}'
...
def _search_citation_soup(soup):
    ...
    while True:
        #print len (soup.findAll('div','gsc_1usr'))
...
def _search_journal_soup(soup):
    """ Generator that returns Top Publications objects from the metrics page"""
    pubTable = soup.find(id = 'gs_cit_list_table').findAll('tr')
    while True:
        for i in range(1,len(pubTable)):
            yield TopJournal(pubTable[i])
        if i == len(pubTable):
...
def _search_pub_soup(soup):
    #print ("Extracting articles")
    pubTable = soup.find(id = 'gs_cit_list_table').findAll('tr')
    while True:
        for i in range(1,len(pubTable)):
            yield Article(pubTable[i])
        nextbutton = soup.find(class_='gs_btnPR gs_in_ib gs_btn_half gs_btn_srt')
        if nextbutton and 'disabled' not in nextbutton.attrs:
            next_url = nextbutton['onclick'][17:-1]
            next_url = codecs.getdecoder("unicode_escape")(next_url)[0]
...
class Article(object):
    def __init__(self,__data):
        self.pub = dict()
        if __data.find('td',class_ = 'gs_title') is not None:
            self.pub['title'] = __data.find('td',class_ = 'gs_title').span.text
            cit_index = __data.findAll('td',class_ = 'gs_num')
            self.pub['url'] = __data.find('td',class_ = 'gs_title').find('a')['href']
            self.pub['Citedby'] = cit_index[0].text
            self.pub['Year'] = cit_index[1].text
            url = cit_index[0].find('a')['href']
            self.pub['journal_id'] = find_between(url,'venue=','&')

        self.filled = False
    def fill(self):
        self.pubList = list()
        keyword = self.pub['title']
        #print (keyword)
        search_q = search_pubs_query(keyword)
        try:
            for i in search_q:
                print (i)
                self.pubList.append(i)
...
class TopJournal(object):
    """Returns an object for a single journal in the top publications list"""
    def __init__(self,__data):
        self.pub = dict()
        if __data.find('td',class_ = 'gs_title') is not None:
            self.pub['title'] = __data.find('td',class_ = 'gs_title').text
            cit_index = __data.findAll('td',class_ = 'gs_num')
            self.pub['url'] = cit_index[0].find('a')['href']
            self.pub['h5_index'] = cit_index[0].text
            self.pub['h5_median'] = cit_index[1].text
            self.pub['journal_id'] = self.pub['url'].split('venue=')[1]
        self.filled = False
    def fill(self):
        title = self.pub['url']
        soup = _get_soup(title)
        self.articles = _search_pub_soup(soup)
        return self
...
class Publication(object):
    ...
    def __init__(self, __data, pubtype=None):
        ...
        if self.source == 'citations':
            ...
            if year and not (year.text.isspace() or not year.text): # added by Raunak Mundada
            ...
            self.bib['author'] = ",".join([i.strip() for i in authorinfo.text.split(' - ')[0].split(',')]) # Seperate author names by ','
    ...
    def fill(self):
        ...
        if self.source == 'citations':
            ...
            for item in soup.findAll('div', class_='gs_scl'):
                ...
                if key == 'Authors':
                    self.bib['author'] = ",".join([i.strip() for i in val.text.split(',')])
                elif key == 'Volume':
                ...
                elif key == 'Journal': # Extract the name of the journal if it exists
...
class Author(object):
    ...
    def fill(self):
        ...
        citation_indices = soup.findAll('td',class_ = 'gsc_rsb_std')
        self.hIndex = citation_indices[2].text
        self.i_10_index = citation_indices[4].text
        self.hIndex_recent = citation_indices[3].text
        self.i_10_index_recent = citation_indices[5].text
...
def search_topJournal(keyword):
    soup = _get_soup(_PUBLICATIONSEARCH.format(requests.utils.quote(keyword)))
    return _search_journal_soup(soup)

def search_journal(keyword):
    soup = _get_soup(_JOURNALSEARCH.format(requests.utils.quote(keyword)))
    return _search_journal_soup(soup)
